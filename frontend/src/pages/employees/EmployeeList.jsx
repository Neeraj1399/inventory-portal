...
const EmployeeList = () => {
  ...
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [isAssetModalOpen, setIsAssetModalOpen] = useState(false); // New state for asset modal
  const [viewStatus, setViewStatus] = useState("ACTIVE");
  const [selectedEmployee, setSelectedEmployee] = useState(null);
  ...
  const fetchEmployees = async () => {
    try {
      const res = await api.get("/employees");
      // res.data.data contains the aggregation results
      setEmployees(res.data.data || []);
    } catch (err) {
      console.error("Error fetching employees", err);
    } finally {
          ...
          {filteredEmployees.map((emp) => (
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <div
                  className={`flex items-center gap-1.5 text-sm font-medium ${emp.assignedAssetsCount > 0 ? "text-amber-600" : "text-slate-400"}`}
                >
                  <Laptop size={14} />
                  <span>{emp.assignedAssetsCount || 0} Assets</span>
                </div>
                ...
                {emp.status === "ACTIVE" && (
                  <>
                    ...
                    <button
                      ...
                      onClick={() => handleToggleStatus(emp._id, emp.status)}
                      className={`p-2 rounded-xl ${emp.assignedAssetsCount > 0 ? "text-slate-300 cursor-not-allowed" : "text-slate-400 hover:text-red-600 hover:bg-red-50"}`}
                    >
                      <UserMinus size={18} />
                    </button>
                  </>
                )}
                <button className="p-2 text-slate-300 hover:text-blue-600">
                  <ArrowRight size={20} />
                </button>
              </div>
            </div>
