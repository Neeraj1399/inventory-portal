...
const ConsumableConditionModal = ({ isOpen, item, onClose, onRefresh }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Logic: If SCRAP, we decrease totalQuantity.
      // If MAINTENANCE, we might move it to a different field or just log the event.
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3 p-1.5 bg-slate-100 rounded-2xl">
              ...
              <button
                type="button"
                onClick={() =>
                  setFormData({ ...formData, actionType: "SCRAP" })
                }
                className={`flex items-center justify-center gap-2 py-3 rounded-xl text-xs font-black transition-all ${
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 outline-none transition-all font-medium text-sm"
            ...
            <button
              ...
              className="w-full bg-slate-900 hover:bg-slate-800 text-white py-4 rounded-2xl font-black text-sm tracking-widest flex items-center justify-center gap-2 transition-all shadow-xl disabled:opacity-50"
            >
              {loading ? (
